...
class LoginFragment : DefaultFragment(), View.OnClickListener {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        ...
        nnumber = if(thisActivity?.thisDeviceInfo?.getNumber().isNullOrBlank()) { "" } else {thisActivity?.thisDeviceInfo?.getNumber().toString() }
        //nnumber = "01092669963"
    ...
    fun sendLogin() {
        ...
        if(error) {
            val dialog = AlertDialog(activity, AlertDialog.TYPE.OK, "", message, object: AlertDialog.Callback{
                override fun onNo() {
                }
                override fun onYes() {
                }
    ...
    fun getUserInfo(cwnum: String) {
        /*MemberHttp().getUserInfo(cwnum, Http, object: DefaultHttp.Callback {
            ...
            override fun onSuccess(json: Any) {
                ...
                var sCmsBankCode    = if(json.isNull("sCmsBankCode")) "" else json.getString("sCmsBankCode")
                var sCmsBankName    = if(json.isNull("sCmsBankName")) "" else json.getString("sCmsBankName")
                var sCmsBankUser    = if(json.isNull("sCmsBankUser")) "" else AES256.decode(json.getString("sCmsBankUser")).toString()
                var sBankAccount    = json.getString("sBankAccount")

                App.userInfo = UserInfo(catid, vAccount, bzNum, mShipJoin, lvType, name, lv00, lv01, id, lvArea, point, lv03, sComZip,
                    sCartonName, sCartonCode, sCeoName, sCarNum, sUptap, sUpjang, sComName, sComAddr, sEmail, sHP,
                    "", "", nTRSNum, sCartypeName, sCartypeCode, sBankUserName, sBankName, sCarName, sCmsBankCode, sBankAccount, sCmsBankUser, sCmsBankName, "", sAddress, "", sBzType)
    ...
    fun sendFindId() {
        ...
        if(nnumber.isNullOrBlank()) {
            ...
            /*MemberHttp().getFindID(aesnum, Http, object : DefaultHttp.Callback {
                override fun onError(code: DefaultHttp.HttpCode, message: String, hcode: String) {
                    if(code == DefaultHttp.HttpCode.ERROR_201 || code == DefaultHttp.HttpCode.CODE_ERROR) {
                        val dialog = AlertDialog(activity, AlertDialog.TYPE.OK, resources.getString(R.string.dialog_findid_title), message, object : AlertDialog.Callback {
                            override fun onNo() {
                            }
                ...
                override fun onSuccess(json: Any) {
                    ...
                    val dialog = AlertDialog(activity, AlertDialog.TYPE.YESNO, resources.getString(R.string.dialog_findid_title), message, object : AlertDialog.Callback {
                        override fun onNo() {
                        }
